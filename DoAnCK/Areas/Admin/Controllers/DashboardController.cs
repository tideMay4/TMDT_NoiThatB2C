...
namespace DoAnCK.Areas.Admin.Controllers
{
    ...
    public class DashboardController : Controller
    {
        ...
        {
            // Lấy Top các sản phẩm được bán nhiều nhất
            var topProducts = db.CHITIET_DONHANG
                .GroupBy(ct => ct.SANPHAM.TenSP)
                .Select(g => new
                {
                    TenSP = g.Key,
                    TongDaBan = g.Sum(ct => ct.SoLuong)
                })
                .OrderByDescending(x => x.TongDaBan)
                .Take(5)
                .ToList();

            var model = new List<TopSoldCompareViewModel>();

            foreach (var p in topProducts)
            {
                // 1. Lấy danh sách bán ra phân theo cửa hàng
                var storeBreakdown = db.CHITIET_DONHANG
                    .Where(ct => ct.SANPHAM.TenSP == p.TenSP)
                    .GroupBy(ct => ct.SANPHAM.MaTK_Store)
                    .Select(g => new
                    {
                        MaTK = g.Key,
                ...
                string shopAName = "Cửa hàng A";
                int shopAQty = 0;
                string shopBName = "Cửa hàng B";
                int shopBQty = 0;

                // 2. Lấy dữ liệu Top 1 Shop
                if (storeBreakdown.Count > 0)
                {
                    var itemA = storeBreakdown[0];
                    if (itemA.MaTK.HasValue)
                    {
                        var stA = db.CUAHANGs.FirstOrDefault(c => c.MaTK == itemA.MaTK.Value);
                        if (stA != null) shopAName = stA.TenCH;
                    }
                    shopAQty = itemA.SoLuong;
                }

                // 3. Lấy dữ liệu Top 2 Shop
                if (storeBreakdown.Count > 1)
                {
                    var itemB = storeBreakdown[1];
                    if (itemB.MaTK.HasValue)
                    {
                        var stB = db.CUAHANGs.FirstOrDefault(c => c.MaTK == itemB.MaTK.Value);
                        if (stB != null) shopBName = stB.TenCH;
                    }
                    shopBQty = itemB.SoLuong;
                }

                // 4. Tính toán phần trăm hiển thị
                int totalCompare = shopAQty + shopBQty;
                if (totalCompare == 0) totalCompare = 1; // Tránh lỗi chia cho 0 (DivideByZeroException)

                model.Add(new TopSoldCompareViewModel
                {
                    TenSanPham = p.TenSP,
                    TongDaBan = p.TongDaBan,
                    TenCuaHangA = shopAName,
                    SoLuongShopA = shopAQty,
                    TiLeShopA = System.Math.Round((double)shopAQty / totalCompare * 100, 1),
                    TenCuaHangB = shopBName,
                    SoLuongShopB = shopBQty,
                    TiLeShopB = System.Math.Round((double)shopBQty / totalCompare * 100, 1)
                });
            }

            return PartialView("_TopSoldItemsCompare", model);
        }
    }
}